...
bool initializer::try_initialize_for_monocular(data::frame& curr_frm) {
    ...
    std::cout << "Initializing. Number of matches: " << num_matches << std::endl;

    int index_frm_2;
    for (int index_frm_1 = 0; index_frm_1 < init_matches_.size(); index_frm_1++) {
        index_frm_2 = init_matches_.at(index_frm_1);
        if (index_frm_2 < 0) {
            continue;
        }
        const auto& keypt_1 = init_frm_.undist_keypts_.at(index_frm_1);
        const auto& keypt_2 = curr_frm.undist_keypts_.at(index_frm_2);

        // std::cout << "Keypoint in frame " << init_frm_.id_ << ": (" << keypt_1.pt.x << ", " << keypt_1.pt.y << "), matching keypoint in frame " << curr_frm.id_ << ": (" << keypt_2.pt.x << ", " << keypt_2.pt.y << ")" << std::endl;
    }
