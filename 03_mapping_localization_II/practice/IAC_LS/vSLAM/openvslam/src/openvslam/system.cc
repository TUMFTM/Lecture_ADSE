...
Mat44_t system::feed_monocular_frame_1d(const cv::Mat& img, const double timestamp, double& pos_1d, const cv::Mat& mask) {
    assert(camera_->setup_type_ == camera::setup_type_t::Monocular);

    check_reset_request();

    const Mat44_t cam_pose_cw = tracker_->track_monocular_image_1d(img, timestamp, pos_1d, mask);
