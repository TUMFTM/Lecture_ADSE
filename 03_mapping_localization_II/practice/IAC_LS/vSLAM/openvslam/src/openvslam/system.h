...
public:
    ...
    //! Feed a monocular frame to SLAM system to get 1d position
    //! (NOTE: distorted images are acceptable if calibrated)
    Mat44_t feed_monocular_frame_1d(const cv::Mat& img, const double timestamp, double& pos_1d, const cv::Mat& mask = cv::Mat{});
