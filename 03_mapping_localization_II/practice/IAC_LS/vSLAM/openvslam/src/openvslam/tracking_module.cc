...
Mat44_t tracking_module::track_monocular_image_1d(const cv::Mat& img, const double timestamp, double& pos_1d, const cv::Mat& mask) {
    const auto start = std::chrono::system_clock::now();
...
double tracking_module::get_pos_1d() {

    double loop_length = map_db_->get_loop_length();

    if (loop_length == 0) {
        return 0;
    }

    Vec3_t cam_pos = curr_frm_.get_cam_center();

    // find closest keyframe
    data::keyframe* closest_keyframe;
    double distance_to_closest_keyframe = MAXFLOAT;
    for (auto keyframe : map_db_->get_all_keyframes()) {
        Vec3_t keyframe_pos = keyframe->get_cam_center();
        double distance_to_camera = sqrt(pow(cam_pos[0] - keyframe_pos[0], 2.0) + pow(cam_pos[1] - keyframe_pos[1], 2.0) + pow(cam_pos[2] - keyframe_pos[2], 2.0));
        if (distance_to_camera < distance_to_closest_keyframe && keyframe->get_travelled_distance() != 0) {
            distance_to_closest_keyframe = distance_to_camera;
    ...
    // find second closest keyframe
    Vec3_t v1 = closest_keyframe->get_cam_center() - cam_pos;
    data::keyframe* second_closest_keyframe;
    double distance_to_second_closest_keyframe = MAXFLOAT;
    for (auto keyframe : map_db_->get_all_keyframes()) {
        Vec3_t keyframe_pos = keyframe->get_cam_center();
        double distance_to_camera = sqrt(pow(cam_pos[0] - keyframe_pos[0], 2.0) + pow(cam_pos[1] - keyframe_pos[1], 2.0) + pow(cam_pos[2] - keyframe_pos[2], 2.0));
        Vec3_t v2 = keyframe_pos - cam_pos;
        Vec3_t v12 = v1 + v2;
        bool keyframe_is_opposite = sqrt(pow(v12[0], 2) + pow(v12[1], 2) + pow(v12[2], 2)) < distance_to_camera;
        if (distance_to_camera < distance_to_second_closest_keyframe && keyframe_is_opposite && keyframe->get_travelled_distance() != 0) {
            distance_to_second_closest_keyframe = distance_to_camera;
            second_closest_keyframe = keyframe;
        }
    }

    if (second_closest_keyframe == nullptr) {
        return 0;
    }

    // return weighted average of keyframes
    double ratio = distance_to_closest_keyframe / (distance_to_second_closest_keyframe + distance_to_closest_keyframe);
    return (ratio * second_closest_keyframe->get_travelled_distance() + (1 - ratio) * closest_keyframe->get_travelled_distance()) / loop_length;
