...
public:
    ...
    //! Track a monocular frame
    //! (NOTE: distorted images are acceptable if calibrated)
    Mat44_t track_monocular_image_1d(const cv::Mat& img, const double timestamp, double& pos_1d, const cv::Mat& mask = cv::Mat{});
...
protected:
    ...
    //! Find current 1d position on closed loop for IAC localization
    double get_pos_1d();
