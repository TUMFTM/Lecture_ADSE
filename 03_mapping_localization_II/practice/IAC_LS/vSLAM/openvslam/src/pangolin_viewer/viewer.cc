...
viewer::viewer(const std::shared_ptr<openvslam::config>& cfg, openvslam::system* system,
    ...
    : system_(system), frame_publisher_(frame_publisher), map_publisher_(map_publisher),
      ...
      grid_interval_ratio_(cfg->yaml_node_["PangolinViewer.grid_interval_ratio"].as<float>(0.1)),
...
void viewer::draw_horizontal_grid() {
    ...
    const float interval_ratio = grid_interval_ratio_;
    const float grid_min = -100.0f * interval_ratio;
    const float grid_max = 100.0f * interval_ratio;
